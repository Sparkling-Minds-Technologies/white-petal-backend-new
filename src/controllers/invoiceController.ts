...
import { transporter } from "../lib/Utils/emailConfig";
...
export const createInvoice = async (req: AuthRequest, res: Response): Promise<void> => {
    if (!req.user || !req.user._id) {
        res.status(ResponseCode.UNAUTHORIZED).json({ message: "Unauthorized: User not found in request" });
        return;
    }
    ...
    if (!dueDate || !instructorDetails || !companyDetails || !services ||
        ...
        res.status(ResponseCode.BAD_REQUEST).json({ message: "All required fields must be provided" });
        return;
    }
    ...
    try {
        ...
        transporter.sendMail(mailOptions, (err) => {
            if (err) {
                res.status(ResponseCode.SERVER_ERROR).json({ message: "Invoice created but email not sent", error: err.message });
            } else {
                res.status(201).json({
...
export const getInvoiceById = (req: AuthRequest, res: Response): void => {
    InvoiceModel.findById(req.params.invoiceId)
        .then((invoice) => {
            if (!invoice) {
                res.status(ResponseCode.NOT_FOUND).json({ message: "Invoice not found" });
                return;
            }
            res.status( ResponseCode.SUCCESS).json(invoice);
        })
...
export const getInvoices = (req: AuthRequest, res: Response): void => {
    InvoiceModel.find()
        .then((invoices) => res.status( ResponseCode.SUCCESS).json(invoices))
        .catch((error) => res.status(ResponseCode.SERVER_ERROR).json({ message: error.message }));
};
...
export const updateInvoice = (req: AuthRequest, res: Response): void => {
    InvoiceModel.findByIdAndUpdate(req.params.invoiceId, req.body, { new: true, runValidators: true })
        .then((updatedInvoice) => {
            ...
            res.status( ResponseCode.SUCCESS).json(updatedInvoice);
        })
        .catch((error) => res.status(ResponseCode.BAD_REQUEST).json({ message: error.message }));
};
...
export const deleteInvoice = (req: AuthRequest, res: Response): void => {
    InvoiceModel.findByIdAndDelete(req.params.invoiceId)
        .then((deletedInvoice) => {
            ...
            res.status( ResponseCode.SUCCESS).json({ message: "Invoice deleted successfully" });
        })
        .catch((error) => res.status(ResponseCode.BAD_REQUEST).json({ message: error.message }));
};
