...
export const approveInvoice = (req: Request, res: Response) => {
  InvoiceModel.findById(req.params.id)
    ...
    .then((invoice) => {
      if (!invoice) return res.status(ResponseCode.NOT_FOUND).json({ message: "Invoice not found" });
      if (!invoice.instructor || !invoice.instructor.email) {
        return res.status(ResponseCode.BAD_REQUEST).json({ message: "Instructor email missing" });
      }
