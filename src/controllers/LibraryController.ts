...
export const uploadLibraryVideo = (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (!videoFile || !coverImageFile) {
    res.status(ResponseCode.BAD_REQUEST).json({ error: "Both video and cover image are required." });
    return Promise.resolve();
  }
  ...
  return Promise.all([
    ...
    .catch((error) => {
      res.status(ResponseCode.SERVER_ERROR).json({ error: "Internal server error", details: error });
    });
};
...
export const getLibraryVideo = (req: Request, res: Response): Promise<void> => {
  ...
  LibraryVideo.find(query)
    ...
    .catch((err) => res.status(ResponseCode.SERVER_ERROR).json({ error: err.message }));
...
export const getLibraryVideoById = (req: Request, res: Response): Promise<void> => {
  LibraryVideo.findById(req.params.id)
    .then((video) => video ? res.json({ video }) : res.status(ResponseCode.NOT_FOUND).json({ message: 'Library Video not found' }))
    .catch((err) => res.status(ResponseCode.SERVER_ERROR).json({ error: err.message }));
...
//         : res.status(ResponseCode.NOT_FOUND).json({ message: 'Library Video not found' })
//     )
//     .catch((err) => res.status(ResponseCode.SERVER_ERROR).json({ error: err.message }));
...
export const deleteLibraryVideo = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status( ResponseCode.SUCCESS).json({ message: "Library video moved to Recycle Bin successfully" });
  } catch (error: any) {
    console.error("Error in deleteLibraryVideo:", error.message);
    res.status(ResponseCode.SERVER_ERROR).json({ message: "Error while deleting library video", error: error.message });
  }
};
