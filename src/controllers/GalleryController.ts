...
export const uploadImage = (req: AuthRequest, res: Response) => {
  if (!req.file || !req.body.title || !req.body.schoolName) {
    res.status(ResponseCode.BAD_REQUEST).json({ message: "file, title, and schoolName are required" });
    return;
  }

  if (!req.user || !req.user._id) {
    res.status(ResponseCode.FORBIDDEN).json({ message: "Unauthorized: User must be logged in" });
    return;
  }
  ...
  Gallery.create({
    ...
    .catch((err) =>
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Upload failed", error: err })
    );
};
/**
...
export const approveImage = (req: AuthRequest, res: Response): void => {
  ...
  Gallery.findByIdAndUpdate(req.params.id, { approved: true }, { new: true })
    .then((image) => {
      if (!image) return res.status(ResponseCode.NOT_FOUND).json({ message: "Image not found." });
      res.json({ message: "Image approved.", image });
    })
    .catch((err) =>
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Approval failed.", error: err })
    );
};
...
export const rejectImage = (req: AuthRequest, res: Response): void => {
  ...
  Gallery.findByIdAndUpdate(req.params.id, { approved: false }, { new: true })
    ...
    .catch((err) =>
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Rejection failed.", error: err })
    );
};
...
export const getGallery = (req: AuthRequest, res: Response): void => {
  ...
  Gallery.find()
    ...
    .catch((err) =>
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Error fetching images.", error: err })
    );
};
...
export const updateImage = (req: AuthRequest, res: Response): void => {
  ...
  if (!imageUrl && !title && !schoolName) {
    res.status(ResponseCode.BAD_REQUEST).json({ message: "No data provided for update." });
  }
  ...
  Gallery.findByIdAndUpdate(
    ...
    .then((updatedImage) => {
      if (!updatedImage)
        return res.status(ResponseCode.NOT_FOUND).json({ message: "Image not found." });
      res.json({ message: "Image updated successfully.", image: updatedImage });
    })
    .catch((err) =>
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Update failed.", error: err })
    );
};
...
export const deleteImage = (req: AuthRequest, res: Response): void => {
  Gallery.findById(req.params.id)
    ...
    .catch((err) => {
      res.status(ResponseCode.SERVER_ERROR).json({ message: "Delete failed.", error: err });
    });
};
