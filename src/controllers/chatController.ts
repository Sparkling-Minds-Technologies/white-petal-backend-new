...
export const sendMessage = (req: Request, res: Response): void => {
  ...
  if (!receiverId || typeof receiverId !== "string") {
    res.status(ResponseCode.BAD_REQUEST).json({ error: "Invalid receiverId" });
    return;
  }

  if (!text || typeof text !== "string") {
    res.status(ResponseCode.BAD_REQUEST).json({ error: "Message text is required" });
    return;
  }
  ...
  Promise.all([User.findById(senderId), User.findById(receiverId)])
    ...
    .catch((error: any) => {
      ...
      res.status(ResponseCode.SERVER_ERROR).json({ error: error.message || "Server error" });
    });
};
...
export const getMessageById = (req: Request, res: Response): void => {
  ...
  if (!mongoose.Types.ObjectId.isValid(messageId)) {
    res.status(ResponseCode.BAD_REQUEST).json({ error: "Invalid message ID" });
    return;
  }
  ...
  Message.findById(messageId)
    ...
    .then((message) => {
      if (!message) {
        res.status(ResponseCode.NOT_FOUND).json({ error: "Message not found" });
        return;
      }
      ...
      if (!isParticipant) {
        res.status(ResponseCode.FORBIDDEN).json({ error: "Access denied" });
        return;
      }
    ...
    .catch((error: any) => {
      ...
      res.status(ResponseCode.SERVER_ERROR).json({ error: "Internal server error" });
    });
};
...
export const getUsersForSidebar = (req: Request, res: Response) => {
  ...
  User.find(filter)
    ...
    .then((filteredUsers) => {
      res.status( ResponseCode.SUCCESS).json(filteredUsers);
    })
    .catch((error: any) => {
...
export const getConversation = (req: Request, res: Response): void => {
  ...
  if (!mongoose.Types.ObjectId.isValid(receiverId)) {
    res.status(ResponseCode.BAD_REQUEST).json({ error: "Invalid receiver ID" });
    return;
  }
  ...
  User.findById(receiverId)
    ...
    .then((messages) => {
      res.status( ResponseCode.SUCCESS).json(messages);
    })
    .catch((error: any) => {
      console.error("Error fetching conversation:", error);
      res.status(ResponseCode.SERVER_ERROR).json({ error: error.message || "Internal server error" });
    });
};
