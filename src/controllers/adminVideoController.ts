    ...
    VideoModel.findById(req.params.id)
        ...
        .then((video) => {
            if (!video || !video.uploadedBy) {
                res.status(ResponseCode.NOT_FOUND).json({ message: "Video or instructor details not found" });
                return;
            }
            AdminModel.findOne()
                .then((admin) => {
                    ...
                    VideoModel.findByIdAndUpdate(req.params.id, {
                        ...
                        .then((updatedVideo) => {
                            if (!updatedVideo) {
                                res.status(ResponseCode.BAD_REQUEST).json({ message: "Video update failed" });
                                return;
                            }
                            const invoiceData = {
                ...
                .catch((error) => res.status(ResponseCode.SERVER_ERROR).json({ message: "Failed to fetch admin details" }));
        })
...
export const rejectVideo = (req: Request, res: Response): void => {
    ...
    if (!rejectionReason) {
        res.status(ResponseCode.BAD_REQUEST).json({ message: "Rejection reason is required" });
        return;
    }
    ...
    VideoModel.findById(req.params.id)
        ...
        .then((video) => {
            ...
            AdminModel.findOne()
                .then((admin) => {
                    if (!admin) {
                        res.status(ResponseCode.SERVER_ERROR).json({ message: "Admin details not found" });
                        return;
                    }
                ...
                .catch((error) => {
                    res.status(ResponseCode.SERVER_ERROR).json({ message: "Failed to fetch admin details" });
                });
        })
        .catch((error) => {
